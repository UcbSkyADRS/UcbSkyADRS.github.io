...
The Uniform Progress algorithm proposed from the [NSDI'24 paper](https://www.usenix.org/conference/nsdi24/presentation/wu-zhanghao) follows a simple idea: use on-demand instances early and stay close to the **target progress line** (illustrated in [Figure 1](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21))**,** which represents the ideal steady progress rate**,** ensuring sufficient slack time to fall back to on-demand instances.
...
To test whether an ADRS can discover a better-performing solution, we apply OpenEvolve to this problem, using the greedy policy mentioned [above](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21) as the starting program (we also tried starting from Uniform Progress; see observations [below](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21)). We evolve the policy over 400 iterations, which takes 5 hours and costs under $20, using a model ensemble of 80% Gemini 2.5 Pro and 20% GPT-4o. We use only ~30% of the total evaluation traces during evolution, to prevent overfitting. For evaluation, we measure the final algorithm using traces from the original paper that span different deadlines, cold start delays, regions, and GPU types.
...
The evolved algorithm shows three main innovations that address Uniform Progress's limitations (shown in [Figure 2](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21)):
...
Over 400 iterations, OpenEvolve gradually refined when to use spot instances ([Figure 3.](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21)). Early generations learned **window-based pattern recognition**, tracking recent spot availability to distinguish between periods when spots were plentiful versus scarce. As evolution progressed, the policy developed **adaptive decision thresholds** that adjust based on how volatile the spot market has been recently.
...
The evolution unfolded through a few key breakthroughs (shown in [Figure 5.](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21)):
...
When tested on a subset of workloads, they pick up workload-specific patterns instead of real strategies. In this case, we found that testing on just one spot pattern caused the evolved policy fail on other workloads (Shown in [Figure 6.](https://www.notion.so/Automating-Algorithm-Discovery-A-Case-Study-in-Spot-Instance-Scheduling-29c8d1e34044814f84aceefa5cfe84c1?pvs=21)). The fix is simple: use broader, more varied test sets so evolution learns to generalize, not memorize.
